...
public class SlashSword : MonoBehaviour {
    ...
    public ParticleSystem ExcaliburSlash;
	...
	void Update () {
		...
		if (slash01anim == true || slash02anim == true) {
            ...
            //ExcaliburSlash.SetActive(true);
		}
		else if (slash01anim == false) {
			slashTrail.SetActive (false);
           // ExcaliburSlash.SetActive(false);
        }
    ...
    public void PlaySlashOn()
    {
        StartCoroutine(PlaySwordBlueSlash());
    }
    IEnumerator PlaySwordBlueSlash()
    {
        ExcaliburSlash.Play();
